...
[Figure 1 - The Yara rule](assets/yara_rule)

## Solution
The condition in the Yara rule describes a file that likely contains the flag.  
Several types of conditions repeat throughout the rule, so I used regex to handle each type.

To solve it, I wrote a script called `bruteforce_hash.py` to brute-force the 2-byte inputs for the hashing algorithms specified in the rule, and another script, `solve.py`, to fully reverse it and create the matching file.  
I used the two scripts separately and hardcoded the values in `solve.py`, though there are multiple ways to reverse the Yara rule.  
...
# '''
# - uint8(7) & 128 == 0
# = unsinged integer of 8 bits (1 byte) at ((offset 7) & 0b10000000) == 0
# = at this byte, value needs to be 0
# '''

# '''
# - uint32(41) + 404880684[left_param] == 1699114335[right_param]
# = at this 4 bytes, do (right_param - left_param)
...
def handle_uint32_op(file: bytearray, matches_arr: list):
    # match[0] = offset
    # match[1] = operator
    # match[2] = left_param
    # math[3] = right_param
    for match in matches_arr:
        offset = int(match[0])
        op = match[1]
        left_param = int(match[2])
        right_param = int(match[3])
        
        calc = 0
        if (op == '+'):
            calc = right_param - left_param
            ...
            raise NotImplementedError(f'\'{op}\' is not expected.')
        
        byte_representation = (calc).to_bytes(4, byteorder='little')
        file[offset:offset+4] = byte_representation
            

def handle_uint8_op(data: bytearray, matches: list):
    # match[0] = offset
    # match[1] = op
    # match[2] = left_param
    # match[3] = right_param
...
def bruteforce_remaining_bytes_with_file_hash(data: bytearray):
    file_hash = "b7dc94ca98aa58dabb5404541c812db2"
    for b2 in range(1, 128):
        for b3 in range(1, 128):
                    data[21] = b2
                    data[26] = b3
                    
                    print(f'{data}\n')
                    
                    curr_hash = hashlib.md5(data).hexdigest()
                    if curr_hash == file_hash:
                        print("BRUTEFORCE COMPLETE, CHECK THE FILE :D")
                        return                             

def handle_md5(data: bytearray):
    data[0:2] = b'ru'
    data[76:78] = b'io'
    data[50:52] = b'3A'
    data[32:34] = b'ul'

def handle_sha256(data: bytearray):
    data[56:58] = b'fl'
    data[14:16] = b' s'
    
def handle_crc32(data: bytearray):
    data[78:80] = b'n:'
    data[8:10] = b're'
    data[34:36] = b'eA'
    data[63:65] = b'n.'
...
**_Final file_**: `rule flareon { strings: $f = "1RuleADayK33p$Malw4r3Aw4y@flare-on.com" condition: $f }`
